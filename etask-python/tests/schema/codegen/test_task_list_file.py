...
def test_render_typelist_no_trailing_comma():
    out = TaskListFile.render([
        ("../tasks/blink.hpp", "sys::blink", Tier.POLLED),
        ("../tasks/motor/spin.hpp", "sys::motor::spin", Tier.POLLED),
    ])
...
def test_tasks_are_split_by_tier():
    out = TaskListFile.render([
        ("a.hpp", "sys::cmd", Tier.INSTANT),
        ("b.hpp", "sys::once", Tier.ONESHOT),
        ("c.hpp", "sys::poll", Tier.POLLED),
        ("d.hpp", "sys::hold", Tier.STATEFUL),
    ])
