...
_BASE = {
    "system": {
        "spin": {"type": "polled_task", "uid": 1, "params": {"duty": "uint8"}},
        "read": {"type": "oneshot_task", "uid": 2, "returns": {"value": "float"}},
    },
...
@pytest.mark.parametrize("label,mutate", [
    ...
    ("uid",            lambda d: d["system"]["spin"].__setitem__("uid", 7)),
    ...
    ("uid width",      lambda d: d["system"]["read"].__setitem__("uid", 300)),
...
def test_canonical_orders_tasks_by_uid_not_by_declaration(tmp_path):
    data = {"system": {
        "late": {"type": "polled_task", "uid": 9},
        "early": {"type": "polled_task", "uid": 1},
    }}
    text = Fingerprint.canonical(Tree.build(write(tmp_path, data)))
    assert text.index("task 1 early") < text.index("task 9 late")
