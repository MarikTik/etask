# tools/tests/etask.schema/test_tree.py
...
from etask.schema.models.node import Kind
from etask.schema.errors import (
    DuplicateUidError,
...
def test_scope_and_task_kinds(tmp_path):
    root = build(tmp_path, {
        "arm": {"type": "scope", "children": {
            "grab": {"type": "polled_task", "params": {"force": "uint8"}}
        }}
    })
    arm = find(root, "arm")
...
def test_param_order_preserved(tmp_path):
    root = build(tmp_path, {
        "t": {"type": "polled_task", "params": {"a": "int", "b": "float", "c": "uint8"}}
    })
...
def test_returns_positional_list(tmp_path):
    root = build(tmp_path, {
        "t": {"type": "polled_task", "returns": ["float", "float", "int"]}
    })
...
def test_injected_scope_is_parent(tmp_path):
    root = build(tmp_path, {
        "leg": {"type": "scope", "children": {
            "calibrate": {"type": "polled_task", "params": {}}
        }},
        "reboot": {"type": "polled_task", "params": {}}
    })
...
def test_abstract_scope_expands_per_instance(tmp_path):
    root = build(tmp_path, {
        "motor": {"type": "abstract_scope", "instances": ["m1", "m2", "m3"], "children": {
            "on": {"type": "polled_task", "params": {"intensity": "uint8"}}
        }}
    })
...
def test_generated_uids_are_deterministic(tmp_path):
    data = {"a": {"type": "scope", "children": {"t": {"type": "polled_task", "params": {}}}}}
...
def test_bad_identifier_raises(tmp_path):
    with pytest.raises(InvalidIdentifierError):
        build(tmp_path, {"2bad": {"type": "polled_task", "params": {}}})


def test_reserved_keyword_identifier_raises(tmp_path):
    with pytest.raises(InvalidIdentifierError):
        build(tmp_path, {"class": {"type": "polled_task", "params": {}}})


def test_unknown_type_raises(tmp_path):
    with pytest.raises(UnknownTypeError):
        build(tmp_path, {"t": {"type": "polled_task", "params": {"x": "int128"}}})
...
def test_string_type_rejected_in_params_and_returns(tmp_path):
    ...
    with pytest.raises(UnknownTypeError):
        build(tmp_path, {"t": {"type": "polled_task", "params": {"name": "string"}}})
    with pytest.raises(UnknownTypeError):
        build(tmp_path, {"t": {"type": "polled_task", "returns": {"label": "string"}}})
...
def test_task_with_children_raises(tmp_path):
    with pytest.raises(SchemaShapeError):
        build(tmp_path, {
            "t": {"type": "polled_task", "params": {}, "children": {
                "x": {"type": "polled_task", "params": {}}
            }}
        })
...
def test_explicit_uid_in_abstract_scope_raises(tmp_path):
    with pytest.raises(SchemaShapeError):
        build(tmp_path, {
            "motor": {"type": "abstract_scope", "instances": ["m1"], "children": {
                "on": {"type": "polled_task", "uid": 3, "params": {}}
            }}
        })
...
def test_yaml_json_examples_equivalent():
    root = str(Path(__file__).resolve().parents[3] / "schema")
