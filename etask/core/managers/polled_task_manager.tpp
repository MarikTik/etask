...
namespace etask::core::managers {
    ...
    polled_task_manager<Tasks...>::polled_task_manager(std::size_t max_task_load)
    {
    ...
    status_code polled_task_manager<Tasks...>::register_task(
    ...
    template <typename... Tasks>
    status_code polled_task_manager<Tasks...>::complete_task(task_uid_t uid, completion_reason reason)
    {
    ...
    template <typename... Tasks>
    void polled_task_manager<Tasks...>::update()
    {
        _garbage.reset();
    ...
    template <typename... Tasks>
    typename polled_task_manager<Tasks...>::task_iterator
    polled_task_manager<Tasks...>::find(task_uid_t uid) noexcept
    {
    ...
    template <typename... Tasks>
    constexpr std::size_t polled_task_manager<Tasks...>::capacity_of(raw_uid_t raw_uid) noexcept
    {
        std::size_t count = 0;
    ...
    template <typename... Tasks>
    constexpr bool polled_task_manager<Tasks...>::owns(raw_uid_t raw_uid) noexcept
    {
        return ((detail::raw_uid_extractor<typename reg_t<Tasks>::type>::value == raw_uid) or ...);
    }

    template <typename... Tasks>
    polled_task_manager<Tasks...>::task_info::task_info(
    ...
    template <typename... Tasks>
    constexpr status_code polled_task_manager<Tasks...>::reply_status(completion_reason reason) noexcept
    {
        if (reason == completion_reason::finished)
