...
namespace etask::core::protocol {
    ...
    request<Packet, TaskUid>::request(const Packet& packet) noexcept
        : _packet{packet}
    {
    }

    template<typename Packet, typename TaskUid>
    directive::operation request<Packet, TaskUid>::command() const noexcept
    {
        std::byte raw;
    ...
    template<typename Packet, typename TaskUid>
    completion_reason request<Packet, TaskUid>::reason() const noexcept
    {
        std::byte raw;
        std::memcpy(&raw, _packet.payload, sizeof(raw));
    ...
    template<typename Packet, typename TaskUid>
    TaskUid request<Packet, TaskUid>::uid() const noexcept
    {
        TaskUid uid;
    ...
    template<typename Packet, typename TaskUid>
    etools::memory::buffer_view request<Packet, TaskUid>::args() const noexcept
    {
