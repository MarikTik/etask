...
*      |  command  |       reason       |
*      +---+---+---+---+---+---+---+---+
*  7..6 : command  (2 bits)  --  directive::wire_command, 4 values used
...
*/
#ifndef ETASK_CORE_PROTOCOL_WIRE_COMMAND_HPP_
#define ETASK_CORE_PROTOCOL_WIRE_COMMAND_HPP_
...
namespace etask::core::protocol {
    ...
    *
    * `wire_command` has exactly 4 values (2 bits); `completion_reason` is
    ...
    * `reason` is only meaningful when `command() == wire_command::complete_task`;
    ...
    */
    ...
    public:
        ...
        * `directive::wire_command` where the type itself is needed.
        */
        enum wire_command : std::uint8_t {
            register_task = 0,
            pause_task    = 1,
            resume_task   = 2,
            complete_task = 3,
        ...
        /**
        * @brief Packs a `wire_command` and a `completion_reason` into one byte.
        *
        ...
        */
        constexpr directive(wire_command cmd, completion_reason reason) noexcept;

        /** @brief Wraps an already-packed byte, e.g. read off `Packet::payload[0]`. */
        explicit constexpr directive(std::byte raw) noexcept;

        /** @brief Extracts the `wire_command` from the high 2 bits. */
        [[nodiscard]] constexpr wire_command command() const noexcept;
...
#include "wire_command.inl"
#endif // ETASK_CORE_PROTOCOL_WIRE_COMMAND_HPP_
