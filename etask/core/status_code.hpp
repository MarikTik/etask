// SPDX-License-Identifier: MIT
/**
* @file status_code.hpp
*
* @brief Defines the status codes used by the etask system for task execution reporting and error handling.
*
* @ingroup etask_core etask::core
*
* @author Mark Tikhonov <mtik.philosopher@gmail.com>
*
* @date 2025-07-09
*
* @copyright
* MIT License
* Copyright (c) 2025 Mark Tikhonov
* See LICENSE file for details.
*/
#ifndef ETASK_CORE_STATUS_CODE_HPP_
#define ETASK_CORE_STATUS_CODE_HPP_
#include <cstdint>

namespace etask::core{
    /**
    * @enum status_code
    * @brief Unified status space used in packets and API returns.
    *
    * @note Codes are partitioned by numeric range for readability and fast checks.
    *       See @ref is_manager_status and @ref is_task_status.
    */
    enum [[nodiscard]] status_code : std::uint8_t {

        /** @name Manager/API status codes (0x00–0x1F) */
        ///@{
        ok                      = 0x00, /**< General success. */
        task_not_registered     = 0x01, /**< Operated on a task that is not registered. */
        task_already_running    = 0x02, /**< Start/resume requested but task is already running. */
        task_already_paused     = 0x03, /**< Pause requested but task is already paused. */
        task_already_resumed    = 0x04, /**< Resume requested but task already marked resumed. */
        task_not_paused         = 0x05, /**< Resume requested but task is not paused. */
        task_not_running        = 0x06, /**< Pause requested but task is not running. */
        invalid_state_transition= 0x07, /**< Illegal state change for current task state. */
        task_already_finished   = 0x08, /**< Operation invalid: task already finished. */
        task_already_concluding = 0x09, /**< Operation invalid: task is already concluding - aborted, or force-completed for a caller-supplied reason. A task concludes once. */

        permission_denied       = 0x0A, /**< Initiator not authorized for this operation. */
        would_block             = 0x0B, /**< Unsafe/forbidden in current context (e.g., ISR). */
        reentrancy_conflict     = 0x0C, /**< Manager API called while update() is active. */

        channel_null            = 0x0D, /**< Null channel pointer provided. */
        channel_error           = 0x0E, /**< Channel failed/backpressure (if detectable). */

        constructor_not_found   = 0x0F, /**< Registry has UID but no constructible entry/signature mismatch. */
        invalid_params          = 0x10, /**< Envelope invalid/unsupported for this task type. */
        out_of_memory           = 0x11, /**< Allocation failure when constructing task. */
        task_limit_reached      = 0x12, /**< This task type's own concurrency cap is reached: every slot its `capacity<Task, N>` reserves is occupied, **and that cap is greater than one**. Other task types may still be startable - see task_budget_exhausted for the whole-tier case. A saturated *single*-slot uid reports duplicate_task instead; the two are the same condition split by cap size, so a caller that means "is this task's own capacity spent" must accept either. @note When a uid's cap and its tier's budget are spent at the same registration, this code wins: the per-uid test runs first. A tier whose budget equals some uid's concurrency therefore never reports task_budget_exhausted for that uid, and raising the task's concurrency will not help - the tier was the binding constraint. See integration/bombardment. */
        duplicate_task          = 0x13, /**< The one slot this uid reserves is occupied: it has no explicit `capacity<Task, N>` (or `concurrency: 1`, the schema default), and an instance is already live. Since that default is what most tasks carry, this - not task_limit_reached - is the ordinary "already running" answer, and it is a *retry-when-free* condition rather than a permanent prohibition. Named for the policy reading ("only one of these may exist"), which for a single-slot uid is indistinguishable from its capacity being spent. */
        task_unknown            = 0x14, /**< Task type UID is unknown to the registry. */
        invalid_completion_reason = 0x15, /**< complete_task called with completion_reason::finished (reserved for natural completion). */
        task_not_pausable       = 0x16, /**< Pause/resume requested for a task whose tier has no suspension: it is live, but is not a stateful_task. */
        task_not_addressable    = 0x17, /**< Directive aimed at an instant_task's uid. Such a command never persists, so there is never an instance to pause, resume, or complete - a structural fact about the uid, not a race. */
        task_budget_exhausted   = 0x18, /**< The owning manager is full: its tier's concurrent-task budget is spent, so no task of any type in that tier can start until one concludes. Distinct from task_limit_reached and duplicate_task, which mean this one uid is saturated while the tier may still have room - the two call for different fixes (raise the tier's `budget:`, versus raise that task's `concurrency:`). @note Reported only when the uid's *own* slots are free, because the per-uid test runs first. If a uid's concurrency equals its tier's budget, that uid can never produce this code even when the tier is genuinely the binding constraint - keep every uid's concurrency strictly below its tier's budget, or the more actionable diagnosis is the one suppressed. Nothing enforces that; see integration/bombardment. */
        schema_mismatch         = 0x19, /**< The peer on this link was built from a different wire schema, so the link refuses task traffic. Established by the fingerprint handshake (see protocol/preamble.hpp) before any request is parsed - not by a request failing to parse, which is what makes it different from invalid_params or task_unknown. Those two say "this frame was wrong"; this one says "every frame from this peer will be wrong, including the ones that appear to parse". The dangerous case is precisely the one that would otherwise look healthy: two builds that agree on frame shape but disagree about which uid means which task would run plausible commands with plausible arguments. Only regenerating one side against the other's schema clears it; a retry never will. Scoped to the link, so a device stays reachable and diagnosable over its other links. */
        task_undefined_on_this_link = 0x1A, /**< The uid names a real task, but this link does not carry it. A link may declare `subsystems:` in the schema, which both shrinks its frames to the widest task it actually carries and makes every other uid unreachable over it. Deliberately distinct from task_unknown: that one says the device has no such task, and sends an operator looking for a missing registration or a stale uid ledger, while this one says the task exists and is reachable - over a different wire. The fix is to send it over the link that carries its subsystem, or to add that subsystem to this link's `subsystems:` and regenerate both ends. Note that regenerating changes the schema fingerprint, so the peer must be regenerated too; see schema_mismatch. */

        internal_error          = 0x1F, /**< Unexpected manager fault. */
        ///@}

        /** @name Task/runtime status codes (0x20–0x3F) */
        ///@{
        task_finished           = 0x20, /**< Task completed successfully (normal termination). */
        task_aborted            = 0x21, /**< Task terminated early by abort request. */

        task_timeout            = 0x22, /**< Task exceeded its time budget/deadline. */
        task_io_error           = 0x23, /**< Task I/O/subsystem failure. */
        task_validation_failed  = 0x24, /**< Task rejected inputs/parameters at runtime. */
        task_dependency_missing = 0x25, /**< Dependency/service required by task unavailable. */
        task_busy               = 0x26, /**< Task refused action due to its own constraints. */
        result_too_large        = 0x27, /**< Task's result does not fit the reply's result region; no result bytes were sent. */
        task_completed_early    = 0x28, /**< Task was force-completed for a caller-supplied reason: it concluded before it would have on its own, but was not aborted. The reason byte says why. */
        ///@}

        /** @name Custom/user-defined status codes (0x70–0xFF) */
        ///@{
        custom_error_start      = 0x70  /**< base for user extensions. */
        ///@}
    };

    /**
    * @brief True if the code belongs to the manager/API range.
    * @param code Status code to test.
    * @return `true` for 0x00–0x1F, otherwise `false`.
    */
    [[nodiscard]] constexpr bool is_manager_status(status_code code) noexcept;

    /**
    * @brief True if the code belongs to the task/runtime range.
    * @param code Status code to test.
    * @return `true` for 0x20–(custom_error_start-1), otherwise `false`.
    */
    [[nodiscard]] constexpr bool is_task_status(status_code code) noexcept;

    /**
    * @brief True if the code belongs to the custom range.
    * @param code Status code to test.
    * @return `true` for 0x70 (custom_error_start-1) - 0xFF, otherwise `false`.
    */
    [[nodiscard]] constexpr bool is_custom_status(status_code code) noexcept;
} // namespace etask::core
#include "status_code.inl"
#endif // ETASK_CORE_STATUS_CODE_HPP_
 