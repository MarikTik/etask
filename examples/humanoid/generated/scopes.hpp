...
#include "../sys/head/context.hpp"
#include "../sys/head/imu/context.hpp"
#include "../sys/arms/context.hpp"
#include "../sys/arms/left/context.hpp"
#include "../sys/arms/right/context.hpp"
...
namespace generated::scopes {
    ...
    */
    ...
    /**
    * @brief The `context` of `head`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::head::context& head() noexcept
    {
        return detail::tree().head;
    }

    /**
    * @brief The `context` of `head.imu`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::head::imu::context& head_imu() noexcept
    {
        return detail::tree().head.imu;
    }

    /**
    * @brief The `context` of `arms`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::arms::context& arms() noexcept
    {
        return detail::tree().arms;
    }

    /**
    * @brief The `context` of `arms.left`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::arms::left::context& arms_left() noexcept
    {
        return detail::tree().arms.left;
    }

    /**
    * @brief The `context` of `arms.right`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::arms::right::context& arms_right() noexcept
    {
        return detail::tree().arms.right;
    }

    /**
    * @brief The `context` of `legs`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::legs::context& legs() noexcept
    {
        return detail::tree().legs;
    }

    /**
    * @brief The `context` of `legs.left`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::legs::left::context& legs_left() noexcept
    {
        return detail::tree().legs.left;
    }

    /**
    * @brief The `context` of `legs.right`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::legs::right::context& legs_right() noexcept
    {
        return detail::tree().legs.right;
    }
