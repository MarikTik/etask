...
namespace generated::links {
    ...
    */
    namespace bench {
        ...
        *
        * 2 fixed + 1 variable: the packed directive byte, the 1-byte uid, and the
        * widest task's arguments.
        *
        * The widest is `polled.count_to` at 1 byte, which is where a surprising number
        * comes from - change that task and this changes.
        */
        inline constexpr std::size_t request_payload_need = 3;

        /**
        * @brief Payload bytes a reply must be able to carry.
        *
        * 2 fixed + 6 variable: the 1-byte uid, the status byte, and the widest result
        * any task can reply with.
        *
        * The widest is `stateful.resumable on task_finished` at 6 bytes, which is where
        * a surprising number comes from - change that task and this changes.
        */
        inline constexpr std::size_t reply_payload_need = 8;

        /**
        * @brief The packet a request travels in.
        *
        * 3 bytes against the reply's 8: this direction is the smaller one, and sizing
        ...
        *
        * 8 bytes against the request's 3: this direction is the larger one, and sizing
        * both to the wider would spend the difference in every buffer for nothing.
        *
        * Its size is the payload requirement plus this link's header, rounded up to a
        * multiple of 8 - computed by the compiler, since only it knows how wide the
