...
#include "../sys/swarm/context.hpp"
#include "../sys/hold/context.hpp"
...
namespace generated::scopes {
    ...
    */
    ...
    /**
    * @brief The `context` of `swarm`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::swarm::context& swarm() noexcept
    {
        return detail::tree().swarm;
    }

    /**
    * @brief The `context` of `hold`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::hold::context& hold() noexcept
    {
        return detail::tree().hold;
    }
...
namespace etask::core {
    ...
    /// @brief `swarm`. @see generated::scopes::swarm
    template<> struct scope_binding<1> {
        [[nodiscard]] static sys::swarm::context& get() noexcept
        { return generated::scopes::swarm(); }
    };

    /// @brief `hold`. @see generated::scopes::hold
    template<> struct scope_binding<2> {
        [[nodiscard]] static sys::hold::context& get() noexcept
        { return generated::scopes::hold(); }
    };
