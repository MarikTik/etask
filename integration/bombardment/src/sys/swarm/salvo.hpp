...
*/
//! etask:end doc file
#ifndef SYS_SWARM_SALVO_HPP_
#define SYS_SWARM_SALVO_HPP_
#include "../task.hpp"
#include "context.hpp"
...
namespace sys::swarm {
    //! etask:doc class e3a0bb59da30
    /**
    * @brief occupy a polled record for a fixed number of ticks
    *
    * The bulk of the tier-filling load: four of the budget's six records.
    * Four rather than six on purpose - a uid capped at the budget can never
    * produce `task_budget_exhausted`, because the manager finds its per-uid
    * cap spent first. The remaining two records are taken by `probe`.
    *
    ...
    */
    //! etask:end doc class
    class salvo : public polled_task {
    public:
        /**
        ...
        */
        salvo(std::uint16_t ticks, context& ctx); //! etask:sig
        ...
        */
        ...
        static constexpr global::task_id uid = global::task_id::swarm_salvo;
    ...
    private:
        /**
        * @brief The scope's counters, so this task can report its own conclusion.
        *
        * Held by reference rather than copied: the counters are the harness's
        * only view of what the manager did with records it has already
        * destroyed, so they have to outlive every task that touches them.
        */
        context& _ctx;

        /// @brief How many `on_execute` calls remain before the task reports itself finished.
