...
namespace generated {
    ...
    */
    inline constexpr std::uint64_t schema_fingerprint = 0xACC3121AB2AB7AD3ULL;
...
namespace generated::links {
    ...
    */
    namespace bench {
        ...
        *
        * 2 fixed + 4 variable: the packed directive byte, the 1-byte uid, and the
        * widest task's arguments.
        *
        * The widest is `keyed.converge` at 4 bytes, which is where a surprising number
        * comes from - change that task and this changes.
        */
        inline constexpr std::size_t request_payload_need = 6;

        /**
        * @brief Payload bytes a reply must be able to carry.
        *
        * 2 fixed + 112 variable: the 1-byte uid, the status byte, and the widest result
        * any task can reply with.
        *
        * The widest is `wide.telemetry on task_finished` at 112 bytes, which is where a
        * surprising number comes from - change that task and this changes.
        */
        inline constexpr std::size_t reply_payload_need = 114;

        /**
        * @brief The packet a request travels in.
        *
        * 6 bytes against the reply's 114: this direction is the smaller one, and sizing
        ...
        *
        * 114 bytes against the request's 6: this direction is the larger one, and
