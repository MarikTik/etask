...
*
* 1 external link: bench. Each becomes a namespace holding two packet types - one per
* direction - plus the constants its channel needs.
*
...
namespace generated {
    ...
    */
    inline constexpr std::uint64_t schema_fingerprint = 0xE92A830652346ADEULL;
...
namespace generated::links {
    ...
    */
    namespace bench {
        ...
        *
        * 2 fixed + 48 variable: the packed directive byte, the 1-byte uid, and the
        * widest task's arguments.
        *
        * The widest is `wide.saturated` at 48 bytes, which is where a surprising number
        * comes from - change that task and this changes.
        */
        inline constexpr std::size_t request_payload_need = 50;

        /**
        * @brief Payload bytes a reply must be able to carry.
        *
        * 2 fixed + 47 variable: the 1-byte uid, the status byte, and the widest result
        * any task can reply with.
        *
        * The widest is `wide.everything on task_finished` at 47 bytes, which is where a
        * surprising number comes from - change that task and this changes.
        */
        inline constexpr std::size_t reply_payload_need = 49;

        /**
        * @brief The packet a request travels in.
        *
        * 50 bytes against the reply's 49: this direction is the larger one, and sizing
        ...
        *
        * 49 bytes against the request's 50: this direction is the smaller one, and
