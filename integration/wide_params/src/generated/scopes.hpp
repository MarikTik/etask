...
#include "../sys/echo/context.hpp"
#include "../sys/mixed/context.hpp"
...
namespace generated::scopes {
    ...
    */
    ...
    /**
    * @brief The `context` of `echo`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::echo::context& echo() noexcept
    {
        return detail::tree().echo;
    }

    /**
    * @brief The `context` of `mixed`.
    *
    * Bound as the scope argument of every task in it (see
    * `etask::core::scoped_task_unpack_adapter`). Inlines to a member offset
    * into the one context tree - there is no lookup and no indirection.
    */
    [[nodiscard]] inline sys::mixed::context& mixed() noexcept
    {
        return detail::tree().mixed;
    }
...
namespace etask::core {
    ...
    /// @brief `echo`. @see generated::scopes::echo
    template<> struct scope_binding<1> {
        [[nodiscard]] static sys::echo::context& get() noexcept
        { return generated::scopes::echo(); }
    };

    /// @brief `mixed`. @see generated::scopes::mixed
    template<> struct scope_binding<2> {
        [[nodiscard]] static sys::mixed::context& get() noexcept
        { return generated::scopes::mixed(); }
    };
