...
*/
//! etask:end doc file
#ifndef SYS_MIXED_AVALANCHE_HPP_
#define SYS_MIXED_AVALANCHE_HPP_
#include "../task.hpp"
#include "context.hpp"
...
namespace sys::mixed {
    ...
    * @brief widths descending 8,4,2,1 then ascending again
    *
    * Every field but the first starts at an offset its own alignment does
    ...
    */
    ...
    public:
        ...
        */
        avalanche(std::uint64_t a, std::uint32_t b, std::uint16_t c, std::uint8_t d, std::uint16_t e, std::uint32_t f, std::uint64_t g, context& ctx); //! etask:sig
        ...
        *           - f : std::uint32_t
        *           - g : std::uint64_t
        *         e.g. `return {a, b, c, d, e, f, g};` - the values are
        ...
        */
        ...
        static constexpr global::task_id uid = global::task_id::mixed_avalanche;
        ...
        */
        using params = etools::meta::typelist<std::uint64_t, std::uint32_t, std::uint16_t, std::uint8_t, std::uint16_t, std::uint32_t, std::uint64_t>;
...
#endif // SYS_MIXED_AVALANCHE_HPP_
