#ifndef PACKET_HEADER_INL_
#define PACKET_HEADER_INL_
...
namespace etask::comm{
    ...
    inline header_t::header_t(uint8_t type, uint8_t version, bool encrypted, bool fragmented, uint8_t priority, flags_t flags, bool validated)
        : _space{
                (static_cast<uint16_t>(type & 0xF) << 12) |
        (static_cast<uint16_t>(version & 0x3) << 10) |
        (static_cast<uint16_t>(encrypted ? 1 : 0) << 9) |
        (static_cast<uint16_t>(fragmented ? 1 : 0) << 8) |
        (static_cast<uint16_t>(priority & 0x7) << 5) |
        (static_cast<uint16_t>(static_cast<uint8_t>(flags) & 0x7) << 2) |
        (static_cast<uint16_t>(validated ? 1 : 0) << 1)
        }
    {
    }
