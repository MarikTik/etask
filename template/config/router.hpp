...
#include "wiring.hpp"
// #include "transport/serial_channel.hpp"   // your transport(s)
...
namespace config {
    ...
    *
    * Watches `serial` for `packet_t`s and hands each to the task manager via
    * `external_channel::dispatch`, which parses the etask request, runs the
    * requested manager operation, and sends any reply back through `serial`.
    *
    * @note `serial` and `external` are the instances you defined in wiring.hpp
    *       when enabling external comms.
    */
    inline auto router = ecomm::router{
        ecomm::on_channel(serial,
            ...
            // `serial` for every packet type its handlers declare. For example:
            //
    ...
    *
    * Call once per `config::loop()`. Returns after the router has dispatched
